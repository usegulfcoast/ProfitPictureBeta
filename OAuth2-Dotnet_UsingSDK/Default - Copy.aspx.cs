        ...
        public async System.Threading.Tasks.Task QboApiCall()
        {
            try
            {
                if ((dictionary.ContainsKey("accessToken")) && (dictionary.ContainsKey("realmId")))
                {
                    ...
                    //QueryService<BalanceSheet> inBalance = new QueryService<BalanceSheet>(serviceContext);
                    //var bsheet = inBalance.ExecuteIdsQuery("SELECT * FROM BalanceSheet");


                    //string[] rptnames = new string[] { "SalesByClientDetailTypeDetail", "CUSTOMER_TYPE_SALES_DET" };

                    reportService.end_date = "2021-03-18";
                    reportService.summarize_column_by = "Total";

                    var report1 = reportService.ExecuteReport(txtrpt.Text);
                    int a;

                    string s = string.Empty;

                    if (txtrpt.Text == "ItemSales")
                    {
                        foreach (var row in report1.Rows)
                        {
                            ...
                            //s += $"{cols[0].value} = {cols[2].value}\r\n";
                            try
                            {
                                decimal d = Convert.ToDecimal(cols[2].value);
                                s += $"{cols[0].value} = {d.ToString("c")}\r\n";
                            }
                            catch (Exception exc)
                            {
                                s += " Cast exception.\r\n";
                    ...
                    if (txtrpt.Text == "GeneralLedger")
                    {

                        //((Intuit.Ipp.Data.Header)row.AnyIntuitObjects[0]).ColData[0].value
                        //((Intuit.Ipp.Data.Summary)row.AnyIntuitObjects[2]).ColData[6].value
